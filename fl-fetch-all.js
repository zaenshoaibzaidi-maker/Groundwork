#!/usr/bin/env node
// fl-fetch-all.js
// Fetches ACS 5-year Census data for all 120 Florida House districts and all
// 40 Florida Senate districts, merges with pre-extracted election data from
// fl-elections-data.json, and saves to fl-all-districts-data.json.
//
// Usage:  node fl-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   fl-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "fl-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "fl-all-districts-data.json");

  console.log("Loading election data from fl-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`fl-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Florida House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in fl-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Florida Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in fl-elections-data.json" },
    };
  }
