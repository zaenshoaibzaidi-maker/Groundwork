#!/usr/bin/env node
// az-fetch-all.js
// Fetches ACS 5-year Census data for all 30 Arizona House districts and all
// 30 Arizona Senate districts, merges with pre-extracted election data from
// az-elections-data.json, and saves to az-all-districts-data.json.
//
// Usage:  node az-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   az-all-districts-data.json (created/overwritten in the same directory)
...
// az-elections-data.json stores districts as arrays of records with a
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "az-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "az-all-districts-data.json");

  console.log("Loading election data from az-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`az-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  console.log("Fetching Census data for House districts 1–30 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~6 seconds\n");

  const houseNums   = Array.from({ length: 30 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Arizona House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in az-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Arizona Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in az-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/30  |  Election: ${houseElecOk}/30`);
