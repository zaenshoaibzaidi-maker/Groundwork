#!/usr/bin/env node
// al-generate-districts.js
// Reads al-all-districts-data.json and generates al-districts.js,
// structured identically to ok-districts.js / ar-districts.js.
//
// Usage: node al-generate-districts.js
// Output: al-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'al-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'al-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('al-all-districts-data.json not found. Run al-fetch-all.js first.');
  process.exit(1);
}
...
function getPartisanSub(elec, incumbentName) {
  ...
  if (elec.unopposed) return `${last} won unopposed in 2022.`;
  return `${last} won with ${Math.round(elec.pct)}% in 2022.`;
}
...
function genEntry(num, districtData, idPrefix, chamberLabel, type) {
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "Alabama ${chamberLabel} District ${num}",
    city: "TBD",
    region: "TBD",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "Alabama ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 105; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'al-hd', 'House', 'state house district'));
}

const senateEntries = [];
for (let i = 1; i <= 35; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'al-sd', 'Senate', 'state senate district'));
}

const output = `/* al-districts.js — Groundwork Alabama District Data
 *
 * Generated by al-generate-districts.js from al-all-districts-data.json.
 * Census: ACS 5-Year 2023 (2019–2023) | Elections: 2022 General (AL SoS)
 * ─────────────────────────────────────────────────────────────────────── */

const DEMO_COLORS = ['#4e9e68','#2563eb','#f59e0b','#ef4444'];

const AL_HOUSE_DISTRICTS = [
${houseEntries.join(',\n')}
];

const AL_SENATE_DISTRICTS = [
${senateEntries.join(',\n')}
];
`;
