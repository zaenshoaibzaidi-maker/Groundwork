#!/usr/bin/env node
// in-fetch-all.js
// Fetches ACS 5-year Census data for all 100 Indiana House districts and all
// 50 Indiana Senate districts, merges with pre-extracted election data from
// in-elections-data.json, and saves to in-all-districts-data.json.
//
// Usage:  node in-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   in-all-districts-data.json (created/overwritten in the same directory)
...
// in-elections-data.json stores house/senate as arrays of { districtId: "in-hd-N" | "in-sd-N", ... }.
function indexByDistrictNumber(districts, prefix) {
  const map = {};
  for (const d of districts || []) {
    const n = d.districtId.slice(prefix.length + 1); // strip "in-hd-" / "in-sd-"
    map[n] = d;
  }
  return map;
}
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "in-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "in-all-districts-data.json");

  console.log("Loading election data from in-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`in-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  const electionData   = JSON.parse(fs.readFileSync(ELECTION_FILE, "utf8"));
  const houseElection  = indexByDistrictNumber(electionData.house, "in-hd");
  const senateElection = indexByDistrictNumber(electionData.senate, "in-sd");
  console.log(
  ...
  console.log("\nFetching Census data for Senate districts 1–50 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~10 seconds\n");

  const senateNums   = Array.from({ length: 50 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Indiana House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in in-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Indiana Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in in-elections-data.json" },
    };
  }
  ...
  console.log(`  Senate — Census: ${senateCensusOk}/50   |  Election: ${senateElecOk}/50`);
