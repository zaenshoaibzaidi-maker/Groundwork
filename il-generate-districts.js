#!/usr/bin/env node
// il-generate-districts.js
// Reads il-all-districts-data.json and generates il-districts.js,
// structured identically to ok-districts.js.
//
// Usage: node il-generate-districts.js
// Output: il-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'il-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'il-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('il-all-districts-data.json not found. Run il-fetch-all.js first.');
  process.exit(1);
}
...
// 2022 Senate winner names come through ALL CAPS; 2024 winner names are already
// mixed case. Title-case everything so the two sources render consistently.
...
// Senate winners come from either the 2022 or 2024 PDF depending on which
// districts were up that year, so the election year is left unstated here.
function getPartisanSub(elec, incumbentName) {
  ...
  if (elec.candidates.length === 1) return `${last} won unopposed.`;
  if (pct === null) return `${last} won.`;
  return `${last} won with ${Math.round(pct)}%.`;
}
...
function genEntry(num, districtData, idPrefix, chamberLabel, type) {
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "Illinois ${chamberLabel} District ${num}",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "Illinois ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 118; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'il-hd', 'House', 'state house district'));
}

const senateEntries = [];
for (let i = 1; i <= 59; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'il-sd', 'Senate', 'state senate district'));
}

const output = `/* il-districts.js — Groundwork Illinois District Data
 *
 * Generated by il-generate-districts.js from il-all-districts-data.json.
 * Census: ACS 5-Year 2023 (2019–2023) | Elections: 2024/2022 General (IL SBE)
 * ─────────────────────────────────────────────────────────────────────── */

const DEMO_COLORS = ['#4e9e68','#2563eb','#f59e0b','#ef4444'];

const IL_HOUSE_DISTRICTS = [
${houseEntries.join(',\n')}
];

const IL_SENATE_DISTRICTS = [
${senateEntries.join(',\n')}
];
`;
