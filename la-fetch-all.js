#!/usr/bin/env node
// la-fetch-all.js
// Fetches ACS 5-year Census data for all 105 Louisiana House districts and all
// 39 Louisiana Senate districts, merges with pre-extracted election data from
// la-elections-data.json, and saves to la-all-districts-data.json.
//
// Usage:  node la-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   la-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "la-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "la-all-districts-data.json");

  console.log("Loading election data from la-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`la-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  console.log("Fetching Census data for House districts 1–105 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~25 seconds\n");

  const houseNums   = Array.from({ length: 105 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  console.log("\nFetching Census data for Senate districts 1–39 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~10 seconds\n");

  const senateNums   = Array.from({ length: 39 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Louisiana House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in la-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Louisiana Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in la-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/105  |  Election: ${houseElecOk}/105`);
  console.log(`  Senate — Census: ${senateCensusOk}/39   |  Election: ${senateElecOk}/39`);
