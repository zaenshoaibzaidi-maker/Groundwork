#!/usr/bin/env node
// ca-fetch-all.js
// Fetches ACS 5-year Census data for all 80 California Assembly districts and
// all 40 California Senate districts, merges with pre-extracted election data
// from ca-elections-data.json, and saves to ca-all-districts-data.json.
//
// Usage:  node ca-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   ca-all-districts-data.json (created/overwritten in the same directory)
...
const STATE_FIPS   = "06"; // California
...
// ca-elections-data.json stores all 120 districts as a single flat array of
// records with `districtId` ("SD-1" / "AD-1") and `chamber` ("senate" /
// "assembly") fields, so split and index them here to match the keyed
// lookup shape the rest of this script expects.
function indexByDistrict(records, chamber) {
  const out = {};
  for (const rec of records || []) {
    if (rec.chamber !== chamber) continue;
    const num = rec.districtId.split("-")[1];
    out[String(Number(num))] = rec;
  }
  return out;
}

async function main() {
  const ELECTION_FILE = path.join(__dirname, "ca-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "ca-all-districts-data.json");

  console.log("Loading election data from ca-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`ca-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  const electionData   = JSON.parse(fs.readFileSync(ELECTION_FILE, "utf8"));
  const houseElection  = indexByDistrict(electionData, "assembly");
  ...
  console.log("Fetching Census data for House districts 1–80 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~20 seconds\n");

  const houseNums   = Array.from({ length: 80 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `California Assembly District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in ca-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `California Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in ca-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/80  |  Election: ${houseElecOk}/80`);
