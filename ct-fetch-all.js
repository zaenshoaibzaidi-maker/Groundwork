#!/usr/bin/env node
// ct-fetch-all.js
// Fetches ACS 5-year Census data for all 151 Connecticut House districts and all
// 36 Connecticut Senate districts, merges with pre-extracted election data from
// ct-elections-data.json, and saves to ct-all-districts-data.json.
//
// Usage:  node ct-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   ct-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "ct-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "ct-all-districts-data.json");

  console.log("Loading election data from ct-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`ct-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  console.log("\nFetching Census data for Senate districts 1–36 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~10 seconds\n");

  const senateNums   = Array.from({ length: 36 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Connecticut House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in ct-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Connecticut Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in ct-elections-data.json" },
    };
  }
  ...
  console.log(`  Senate — Census: ${senateCensusOk}/36   |  Election: ${senateElecOk}/36`);
