#!/usr/bin/env node
// ky-generate-districts.js
// Reads ky-all-districts-data.json and generates ky-districts.js,
// structured identically to ok-districts.js.
//
// Usage: node ky-generate-districts.js
// Output: ky-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'ky-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'ky-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('ky-all-districts-data.json not found. Run ky-fetch-all.js first.');
  process.exit(1);
}

const data = JSON.parse(fs.readFileSync(INPUT, 'utf8'));

// Senate districts up for election in 2024 (odd); the rest (even) were up in 2022.
...
function genEntry(num, districtData, idPrefix, chamberLabel, type, year) {
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "Kentucky ${chamberLabel} District ${num}",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "Kentucky ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 100; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'ky-hd', 'House', 'state house district', 2024));
}

const senateEntries = [];
for (let i = 1; i <= 38; i++) {
  const year = i % 2 === 1 ? 2024 : 2022;
  senateEntries.push(genEntry(i, data.senate[String(i)], 'ky-sd', 'Senate', 'state senate district', year));
}

const output = `/* ky-districts.js — Groundwork Kentucky District Data
 *
 * Generated by ky-generate-districts.js from ky-all-districts-data.json.
 * Census: ACS 5-Year 2023 (2019–2023) | Elections: 2024/2022 General (KY SBE)
 * ─────────────────────────────────────────────────────────────────────── */

const DEMO_COLORS = ['#4e9e68','#2563eb','#f59e0b','#ef4444'];

const KY_HOUSE_DISTRICTS = [
${houseEntries.join(',\n')}
];

const KY_SENATE_DISTRICTS = [
${senateEntries.join(',\n')}
];
`;
