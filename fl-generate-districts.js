#!/usr/bin/env node
// fl-generate-districts.js
// Reads fl-all-districts-data.json and generates fl-districts.js,
// structured identically to ok-districts.js.
//
// Usage: node fl-generate-districts.js
// Output: fl-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'fl-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'fl-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('fl-all-districts-data.json not found. Run fl-fetch-all.js first.');
  process.exit(1);
}
...
function genEntry(num, districtData, idPrefix, chamberLabel, type) {
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "Florida ${chamberLabel} District ${num}",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "Florida ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 120; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'fl-hd', 'House', 'state house district'));
}

const senateEntries = [];
for (let i = 1; i <= 40; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'fl-sd', 'Senate', 'state senate district'));
}

const output = `/* fl-districts.js — Groundwork Florida District Data
 *
 * Generated by fl-generate-districts.js from fl-all-districts-data.json.
 * Census: ACS 5-Year 2023 (2019–2023) | Elections: 2024 General (FL SoS)
 * ─────────────────────────────────────────────────────────────────────── */

const DEMO_COLORS = ['#4e9e68','#2563eb','#f59e0b','#ef4444'];

const FL_HOUSE_DISTRICTS = [
${houseEntries.join(',\n')}
];

const FL_SENATE_DISTRICTS = [
${senateEntries.join(',\n')}
];
`;
