#!/usr/bin/env node
// ar-fetch-all.js
// Fetches ACS 5-year Census data for all 100 Arkansas House districts and all
// 35 Arkansas Senate districts, merges with pre-extracted election data from
// ar-elections-data.json, and saves to ar-all-districts-data.json.
//
// Usage:  node ar-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   ar-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "ar-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "ar-all-districts-data.json");

  console.log("Loading election data from ar-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`ar-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Arkansas House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in ar-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Arkansas Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in ar-elections-data.json" },
    };
  }
