#!/usr/bin/env node
// ia-fetch-all.js
// Fetches ACS 5-year Census data for all 100 Iowa House districts and all
// 50 Iowa Senate districts, merges with pre-extracted election data from
// ia-elections-data.json, and saves to ia-all-districts-data.json.
//
// Usage:  node ia-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   ia-all-districts-data.json (created/overwritten in the same directory)
...
// ia-elections-data.json stores house/senate as arrays of { districtId: "ia-hd-N" | "ia-sd-N", ... }.
function indexByDistrictNumber(districts, prefix) {
  const map = {};
  for (const d of districts || []) {
    const n = d.districtId.slice(prefix.length + 1); // strip "ia-hd-" / "ia-sd-"
    map[n] = d;
  }
  return map;
}
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "ia-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "ia-all-districts-data.json");

  console.log("Loading election data from ia-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`ia-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  const electionData   = JSON.parse(fs.readFileSync(ELECTION_FILE, "utf8"));
  const houseElection  = indexByDistrictNumber(electionData.house, "ia-hd");
  const senateElection = indexByDistrictNumber(electionData.senate, "ia-sd");
  console.log(
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Iowa House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in ia-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Iowa Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in ia-elections-data.json" },
    };
  }
