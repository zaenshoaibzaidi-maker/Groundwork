#!/usr/bin/env node
// oh-fetch-all-house.js
// Fetches ACS 5-year Census data + 2024 election results for all 99 Ohio
// State House districts and saves them to oh-house-data.json.
//
// Usage:  node oh-fetch-all-house.js
// Optional: set CENSUS_API_KEY env var to avoid Census API rate limits.
// Requires: npm install xlsx  (same dep as oh-fetch-elections.js)
// Requires: Node 18+ (native fetch)
//
// The file statewide-race-summary.xlsx must be in the same directory.
// Output: oh-house-data.json (created/overwritten in the same directory)
...
// ══════════════════════════════════════════════════════════════════════════════
//  CENSUS LOGIC  (mirrors oh-fetch-house-district.js, adapted to return data not print)
// ══════════════════════════════════════════════════════════════════════════════
...
// Same variable list as oh-fetch-house-district.js
const CENSUS_VARS = [
...
// ══════════════════════════════════════════════════════════════════════════════
//  ELECTION LOGIC  (mirrors oh-fetch-elections.js, parses all 99 districts at once)
// ══════════════════════════════════════════════════════════════════════════════
...
function loadAllElectionData() {
  ...
  // Same row structure as oh-fetch-elections.js
...
async function main() {
  const OUTPUT_FILE = path.join(__dirname, "oh-house-data.json");
