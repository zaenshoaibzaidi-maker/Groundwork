#!/usr/bin/env node
// va-fetch-all.js
// Fetches ACS 5-year Census data for all 100 Virginia House districts and all
// 40 Virginia Senate districts, merges with pre-extracted election data from
// va-elections-data.json, and saves to va-all-districts-data.json.
//
// Usage:  node va-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   va-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "va-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "va-all-districts-data.json");

  console.log("Loading election data from va-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`va-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Virginia House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in va-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Virginia Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in va-elections-data.json" },
    };
  }
