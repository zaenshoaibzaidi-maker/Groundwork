#!/usr/bin/env node
// nc-fetch-all.js
// Fetches ACS 5-year Census data for all 120 North Carolina House districts and all
// 50 North Carolina Senate districts, merges with pre-extracted election data from
// nc-elections-data.json, and saves to nc-all-districts-data.json.
//
// Usage:  node nc-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   nc-all-districts-data.json (created/overwritten in the same directory)
...
const STATE_FIPS   = "37"; // North Carolina
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "nc-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "nc-all-districts-data.json");

  console.log("Loading election data from nc-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`nc-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  console.log("Fetching Census data for House districts 1–120 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~30 seconds\n");

  const houseNums   = Array.from({ length: 120 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `North Carolina House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in nc-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `North Carolina Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in nc-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/120  |  Election: ${houseElecOk}/120`);
