#!/usr/bin/env node
// al-fetch-all.js
// Fetches ACS 5-year Census data for all 105 Alabama House districts and all
// 35 Alabama Senate districts, merges with pre-extracted election data from
// al-elections-data.json, and saves to al-all-districts-data.json.
//
// Usage:  node al-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   al-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "al-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "al-all-districts-data.json");

  console.log("Loading election data from al-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`al-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Alabama House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in al-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Alabama Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in al-elections-data.json" },
    };
  }
