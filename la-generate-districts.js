#!/usr/bin/env node
// la-generate-districts.js
// Reads la-all-districts-data.json and generates la-districts.js,
// structured identically to ok-districts.js.
//
// Usage: node la-generate-districts.js
// Output: la-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'la-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'la-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('la-all-districts-data.json not found. Run la-fetch-all.js first.');
  process.exit(1);
}
...
function genEntry(num, districtData, idPrefix, chamberLabel, type) {
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "Louisiana ${chamberLabel} District ${num}",
    city: "TBD",
    region: "TBD",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "Louisiana ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 105; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'la-hd', 'House', 'state house district'));
}

const senateEntries = [];
for (let i = 1; i <= 39; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'la-sd', 'Senate', 'state senate district'));
}

const output = `/* la-districts.js — Groundwork Louisiana District Data
 *
 * Generated by la-generate-districts.js from la-all-districts-data.json.
 * Census: ACS 5-Year 2023 (2019–2023) | Elections: 2023 Primary/Runoff (LA SoS)
 * ─────────────────────────────────────────────────────────────────────── */

const DEMO_COLORS = ['#4e9e68','#2563eb','#f59e0b','#ef4444'];

const LA_HOUSE_DISTRICTS = [
${houseEntries.join(',\n')}
];

const LA_SENATE_DISTRICTS = [
${senateEntries.join(',\n')}
];
`;
