#!/usr/bin/env node
// ga-generate-districts.js
// Reads ga-all-districts-data.json and generates ga-districts.js,
// structured identically to ok-districts.js / tx-districts.js.
//
// Usage: node ga-generate-districts.js
// Output: ga-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'ga-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'ga-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('ga-all-districts-data.json not found. Run ga-fetch-all.js first.');
  process.exit(1);
}
...
function genEntry(num, districtData, idPrefix, chamberLabel, type) {
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "Georgia ${chamberLabel} District ${num}",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "Georgia ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 180; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'ga-hd', 'House', 'state house district'));
}

const senateEntries = [];
for (let i = 1; i <= 56; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'ga-sd', 'Senate', 'state senate district'));
}

const output = `/* ga-districts.js — Groundwork Georgia District Data
 *
 * Generated by ga-generate-districts.js from ga-all-districts-data.json.
 * Census: ACS 5-Year 2023 (2019–2023) | Elections: 2024 General (GA SoS)
 * ─────────────────────────────────────────────────────────────────────── */

const DEMO_COLORS = ['#4e9e68','#2563eb','#f59e0b','#ef4444'];

const GA_HOUSE_DISTRICTS = [
${houseEntries.join(',\n')}
];

const GA_SENATE_DISTRICTS = [
${senateEntries.join(',\n')}
];
`;
