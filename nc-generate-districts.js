#!/usr/bin/env node
// nc-generate-districts.js
// Reads nc-all-districts-data.json and generates nc-districts.js,
// structured identically to ok-districts.js.
//
// Usage: node nc-generate-districts.js
// Output: nc-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'nc-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'nc-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('nc-all-districts-data.json not found. Run nc-fetch-all.js first.');
  process.exit(1);
}
...
function genEntry(num, districtData, idPrefix, chamberLabel, type) {
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "North Carolina ${chamberLabel} District ${num}",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "North Carolina ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 120; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'nc-hd', 'House', 'state house district'));
}

const senateEntries = [];
for (let i = 1; i <= 50; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'nc-sd', 'Senate', 'state senate district'));
}

const output = `/* nc-districts.js — Groundwork North Carolina District Data
 *
 * Generated by nc-generate-districts.js from nc-all-districts-data.json.
 * Census: ACS 5-Year 2023 (2019–2023) | Elections: 2024/2022 General (NC SBE)
 * ─────────────────────────────────────────────────────────────────────── */

const DEMO_COLORS = ['#4e9e68','#2563eb','#f59e0b','#ef4444'];

const NC_HOUSE_DISTRICTS = [
${houseEntries.join(',\n')}
];

const NC_SENATE_DISTRICTS = [
${senateEntries.join(',\n')}
];
`;
