#!/usr/bin/env node
// nv-fetch-all.js
// Fetches ACS 5-year Census data for all 42 Nevada Assembly (House) districts
// and all 21 Nevada Senate districts, merges with pre-extracted election data
// from nv-elections-data.json, and saves to nv-all-districts-data.json.
//
// Usage:  node nv-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   nv-all-districts-data.json (created/overwritten in the same directory)
...
// nv-elections-data.json stores districts as arrays of records with a
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "nv-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "nv-all-districts-data.json");

  console.log("Loading election data from nv-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`nv-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  console.log("Fetching Census data for House districts 1–42 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~9 seconds\n");

  const houseNums   = Array.from({ length: 42 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  console.log("\nFetching Census data for Senate districts 1–21 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~5 seconds\n");

  const senateNums   = Array.from({ length: 21 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Nevada Assembly District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in nv-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Nevada Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in nv-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/42  |  Election: ${houseElecOk}/42`);
  console.log(`  Senate — Census: ${senateCensusOk}/21  |  Election: ${senateElecOk}/21`);
