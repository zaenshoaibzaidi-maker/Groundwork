#!/usr/bin/env node
// co-fetch-all.js
// Fetches ACS 5-year Census data for all 65 Colorado House districts and all
// 35 Colorado Senate districts, merges with pre-extracted election data from
// co-elections-data.json, and saves to co-all-districts-data.json.
//
// Usage:  node co-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   co-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "co-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "co-all-districts-data.json");

  console.log("Loading election data from co-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`co-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  for (const [key, value] of Object.entries(electionData)) {
    const hm = key.match(/^HD-(\d+)$/);
    const sm = key.match(/^SD-(\d+)$/);
    if (hm) houseElection[String(parseInt(hm[1], 10))] = value;
    if (sm) senateElection[String(parseInt(sm[1], 10))] = value;
  }
  console.log(
  ...
  console.log("Fetching Census data for House districts 1–65 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~13 seconds\n");

  const houseNums   = Array.from({ length: 65 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Colorado House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in co-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Colorado Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in co-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/65  |  Election: ${houseElecOk}/65`);
