#!/usr/bin/env node
// wi-fetch-all.js
// Fetches ACS 5-year Census data for all 99 Wisconsin Assembly (House) districts
// and all 33 Wisconsin Senate districts, merges with pre-extracted election data
// from wi-elections-data.json, and saves to wi-all-districts-data.json.
//
// Usage:  node wi-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   wi-all-districts-data.json (created/overwritten in the same directory)
...
// wi-elections-data.json stores house/senate as arrays of { districtId: "wi-hd-N" | "wi-sd-N", ... }.
function indexByDistrictNumber(districts, prefix) {
  const map = {};
  for (const d of districts || []) {
    const n = d.districtId.slice(prefix.length + 1); // strip "wi-hd-" / "wi-sd-"
    map[n] = d;
  }
  return map;
}
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "wi-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "wi-all-districts-data.json");

  console.log("Loading election data from wi-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`wi-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  const electionData   = JSON.parse(fs.readFileSync(ELECTION_FILE, "utf8"));
  const houseElection  = indexByDistrictNumber(electionData.house, "wi-hd");
  const senateElection = indexByDistrictNumber(electionData.senate, "wi-sd");
  console.log(
    `  House election districts: ${Object.keys(houseElection).length}`,
    `| Senate election districts: ${Object.keys(senateElection).length}\n`
  );

  console.log("Fetching Census data for Assembly districts 1–99 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~20 seconds\n");

  const houseNums   = Array.from({ length: 99 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  console.log("\nFetching Census data for Senate districts 1–33 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~10 seconds\n");

  const senateNums   = Array.from({ length: 33 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Wisconsin Assembly District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in wi-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Wisconsin Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in wi-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/99  |  Election: ${houseElecOk}/99`);
  console.log(`  Senate — Census: ${senateCensusOk}/33  |  Election: ${senateElecOk}/33`);
