#!/usr/bin/env node
// ri-fetch-all.js
// Fetches ACS 5-year Census data for all 75 Rhode Island House districts and
// all 38 Rhode Island Senate districts, merges with pre-extracted election
// data from ri-elections-data.json, and saves to ri-all-districts-data.json.
//
// Usage:  node ri-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   ri-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "ri-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "ri-all-districts-data.json");

  console.log("Loading election data from ri-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`ri-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Rhode Island House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in ri-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Rhode Island Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in ri-elections-data.json" },
    };
  }
