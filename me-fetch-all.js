#!/usr/bin/env node
// me-fetch-all.js
// Fetches ACS 5-year Census data for all 151 Maine House districts and all
// 35 Maine Senate districts, merges with pre-extracted election data from
// me-elections-data.json, and saves to me-all-districts-data.json.
//
// Usage:  node me-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   me-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "me-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "me-all-districts-data.json");

  console.log("Loading election data from me-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`me-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  const electionData = JSON.parse(fs.readFileSync(ELECTION_FILE, "utf8"));

  // me-elections-data.json stores house/senate as arrays of per-district
  // records (keyed by a "district" number field), not objects keyed by
  // district number — index them here for lookup.
  const houseElection  = {};
  for (const rec of electionData.house  || []) houseElection[String(rec.district)]  = rec;
  const senateElection = {};
  for (const rec of electionData.senate || []) senateElection[String(rec.district)] = rec;
  ...
  console.log("Fetching Census data for House districts 1–151 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~30 seconds\n");

  const houseNums   = Array.from({ length: 151 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  console.log("\nFetching Census data for Senate districts 1–35 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~7 seconds\n");
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Maine House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in me-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Maine Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in me-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/151  |  Election: ${houseElecOk}/151`);
