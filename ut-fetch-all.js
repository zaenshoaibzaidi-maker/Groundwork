#!/usr/bin/env node
// ut-fetch-all.js
// Fetches ACS 5-year Census data for all 75 Utah House districts and all 29
// Utah Senate districts, merges with pre-extracted election data from
// ut-elections-data.json, and saves to ut-all-districts-data.json.
//
// Usage:  node ut-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   ut-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "ut-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "ut-all-districts-data.json");

  console.log("Loading election data from ut-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`ut-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }

  // ut-elections-data.json stores each chamber as an array of
  // { district, winner, party, pct, ... }; re-key by district number.
  const electionData  = JSON.parse(fs.readFileSync(ELECTION_FILE, "utf8"));
  const houseElection  = {};
  for (const d of electionData.house  || []) houseElection[String(d.district)]  = d;
  const senateElection = {};
  for (const d of electionData.senate || []) senateElection[String(d.district)] = d;
  console.log(
    `  House election districts: ${Object.keys(houseElection).length}`,
    `| Senate election districts: ${Object.keys(senateElection).length}\n`
  );

  console.log("Fetching Census data for House districts 1–75 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~20 seconds\n");

  const houseNums   = Array.from({ length: 75 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  console.log("\nFetching Census data for Senate districts 1–29 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~10 seconds\n");

  const senateNums   = Array.from({ length: 29 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Utah House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in ut-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Utah Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in ut-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/75  |  Election: ${houseElecOk}/75`);
  console.log(`  Senate — Census: ${senateCensusOk}/29  |  Election: ${senateElecOk}/29`);
