#!/usr/bin/env node
// sc-generate-districts.js
// Reads sc-all-districts-data.json and generates sc-districts.js,
// structured identically to ok-districts.js.
//
// Usage: node sc-generate-districts.js
// Output: sc-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'sc-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'sc-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('sc-all-districts-data.json not found. Run sc-fetch-all.js first.');
  process.exit(1);
}
...
function genEntry(num, districtData, idPrefix, chamberLabel, type, nextElection) {
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "South Carolina ${chamberLabel} District ${num}",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "South Carolina ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 124; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'sc-hd', 'House', 'state house district', 'November 2026'));
}

const senateEntries = [];
for (let i = 1; i <= 46; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'sc-sd', 'Senate', 'state senate district', 'November 2028'));
}

const output = `/* sc-districts.js — Groundwork South Carolina District Data
 *
 * Generated by sc-generate-districts.js from sc-all-districts-data.json.
 * Census: ACS 5-Year 2023 (2019–2023) | Elections: 2024 General (SC SoS)
 * ─────────────────────────────────────────────────────────────────────── */

const DEMO_COLORS = ['#4e9e68','#2563eb','#f59e0b','#ef4444'];

const SC_HOUSE_DISTRICTS = [
${houseEntries.join(',\n')}
];

const SC_SENATE_DISTRICTS = [
${senateEntries.join(',\n')}
];
`;
