#!/usr/bin/env node
// va-generate-districts.js
// Reads va-all-districts-data.json and generates va-districts.js,
// structured identically to ok-districts.js.
//
// Usage: node va-generate-districts.js
// Output: va-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'va-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'va-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('va-all-districts-data.json not found. Run va-fetch-all.js first.');
  process.exit(1);
}
...
function getPartisanSub(elec, incumbentName) {
  ...
  if (elec.unopposed) return `${last} won unopposed in 2025.`;
  return `${last} won with ${Math.round(elec.pct)}% in 2025.`;
}
...
function genEntry(num, districtData, idPrefix, chamberLabel, type) {
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "Virginia ${chamberLabel} District ${num}",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "Virginia ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 100; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'va-hd', 'House', 'state house district'));
}

const senateEntries = [];
for (let i = 1; i <= 40; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'va-sd', 'Senate', 'state senate district'));
}

const output = `/* va-districts.js — Groundwork Virginia District Data
 *
 * Generated by va-generate-districts.js from va-all-districts-data.json.
 * Census: ACS 5-Year 2023 (2019–2023) | Elections: 2025 General (VA SBE)
 * ─────────────────────────────────────────────────────────────────────── */

const DEMO_COLORS = ['#4e9e68','#2563eb','#f59e0b','#ef4444'];

const VA_HOUSE_DISTRICTS = [
${houseEntries.join(',\n')}
];

const VA_SENATE_DISTRICTS = [
${senateEntries.join(',\n')}
];
`;
