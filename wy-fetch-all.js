#!/usr/bin/env node
// wy-fetch-all.js
// Fetches ACS 5-year Census data for all 62 Wyoming House districts and all
// 31 Wyoming Senate districts, merges with pre-extracted election data from
// wy-elections-data.json, and saves to wy-all-districts-data.json.
//
// Usage:  node wy-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   wy-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "wy-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "wy-all-districts-data.json");

  console.log("Loading election data from wy-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`wy-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  const electionData = JSON.parse(fs.readFileSync(ELECTION_FILE, "utf8"));

  // wy-elections-data.json is a flat object keyed by "HD-01".."HD-62" and
  // "SD-01".."SD-31" — split it into house/senate lookups keyed by plain
  // district number to mirror ar-elections-data.json's shape.
  const houseElection  = {};
  const senateElection = {};
  for (const [key, value] of Object.entries(electionData)) {
    const m = key.match(/^(HD|SD)-(\d+)$/);
    if (!m) continue;
    const n = String(parseInt(m[2], 10));
    if (m[1] === "HD") houseElection[n] = value;
    else senateElection[n] = value;
  }
  console.log(
  ...
  console.log("Fetching Census data for House districts 1–62 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~13 seconds\n");

  const houseNums   = Array.from({ length: 62 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Wyoming House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in wy-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Wyoming Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in wy-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/62  |  Election: ${houseElecOk}/62`);
