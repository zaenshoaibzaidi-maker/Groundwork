#!/usr/bin/env node
// tn-generate-districts.js
// Reads tn-all-districts-data.json and generates tn-districts.js,
// structured identically to ok-districts.js.
//
// Usage: node tn-generate-districts.js
// Output: tn-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'tn-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'tn-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('tn-all-districts-data.json not found. Run tn-fetch-all.js first.');
  process.exit(1);
}
...
// TN election data uses full party names ("Republican", "Democratic")
...
// TN election data: unopposed = only one candidate
...
function genEntry(num, districtData, idPrefix, chamberLabel, type) {
  ...
  return `  {
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "Tennessee ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 99; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'tn-hd', 'House', 'state house district'));
}

const senateEntries = [];
for (let i = 1; i <= 33; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'tn-sd', 'Senate', 'state senate district'));
}

const output = `/* tn-districts.js — Groundwork Tennessee District Data
 *
 * Generated by tn-generate-districts.js from tn-all-districts-data.json.
 * Census: ACS 5-Year 2023 (2019–2023) | Elections: 2024/2022 General (TN SoS)
 * ─────────────────────────────────────────────────────────────────────── */

const DEMO_COLORS = ['#4e9e68','#2563eb','#f59e0b','#ef4444'];

const TN_HOUSE_DISTRICTS = [
${houseEntries.join(',\n')}
];

const TN_SENATE_DISTRICTS = [
${senateEntries.join(',\n')}
];
`;
