#!/usr/bin/env node
// nj-fetch-all.js
// Fetches ACS 5-year Census data for all 40 New Jersey General Assembly
// districts and all 40 New Jersey Senate districts, merges with
// pre-extracted election data from nj-elections-data.json, and saves to
// nj-all-districts-data.json.
//
// Usage:  node nj-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   nj-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "nj-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "nj-all-districts-data.json");

  console.log("Loading election data from nj-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`nj-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  const electionData   = JSON.parse(fs.readFileSync(ELECTION_FILE, "utf8"));
  const houseElection  = electionData.assembly || {};
  ...
  console.log("Fetching Census data for General Assembly districts 1–40 (lower chamber)...");
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `New Jersey General Assembly District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in nj-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `New Jersey Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in nj-elections-data.json" },
    };
  }
