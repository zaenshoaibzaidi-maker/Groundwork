#!/usr/bin/env node
// pa-generate-districts.js
// Reads pa-all-districts-data.json and generates pa-districts.js,
// structured identically to ok-districts.js.
//
// Usage: node pa-generate-districts.js
// Output: pa-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'pa-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'pa-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('pa-all-districts-data.json not found. Run pa-fetch-all.js first.');
  process.exit(1);
}
...
function genEntry(num, districtData, idPrefix, chamberLabel, type) {
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "Pennsylvania ${chamberLabel} District ${num}",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "Pennsylvania ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 203; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'pa-hd', 'House', 'state house district'));
}

const senateEntries = [];
for (let i = 1; i <= 50; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'pa-sd', 'Senate', 'state senate district'));
}

const output = `/* pa-districts.js — Groundwork Pennsylvania District Data
 *
 * Generated by pa-generate-districts.js from pa-all-districts-data.json.
 * Census: ACS 5-Year 2023 (2019–2023) | Elections: 2024/2022 General (PA SoS)
 * ─────────────────────────────────────────────────────────────────────── */

const DEMO_COLORS = ['#4e9e68','#2563eb','#f59e0b','#ef4444'];

const PA_HOUSE_DISTRICTS = [
${houseEntries.join(',\n')}
];

const PA_SENATE_DISTRICTS = [
${senateEntries.join(',\n')}
];
`;
