#!/usr/bin/env node
// sc-fetch-all.js
// Fetches ACS 5-year Census data for all 124 South Carolina House districts and all
// 46 South Carolina Senate districts, merges with pre-extracted election data from
// sc-elections-data.json, and saves to sc-all-districts-data.json.
//
// Usage:  node sc-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   sc-all-districts-data.json (created/overwritten in the same directory)
...
const STATE_FIPS   = "45"; // South Carolina
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "sc-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "sc-all-districts-data.json");

  console.log("Loading election data from sc-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`sc-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  console.log("Fetching Census data for House districts 1–124 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~30 seconds\n");

  const houseNums   = Array.from({ length: 124 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  console.log("\nFetching Census data for Senate districts 1–46 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~12 seconds\n");

  const senateNums   = Array.from({ length: 46 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `South Carolina House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in sc-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `South Carolina Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in sc-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/124  |  Election: ${houseElecOk}/124`);
  console.log(`  Senate — Census: ${senateCensusOk}/46   |  Election: ${senateElecOk}/46`);
