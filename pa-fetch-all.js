#!/usr/bin/env node
// pa-fetch-all.js
// Fetches ACS 5-year Census data for all 203 Pennsylvania House districts and all
// 50 Pennsylvania Senate districts, merges with pre-extracted election data from
// pa-elections-data.json, and saves to pa-all-districts-data.json.
//
// Usage:  node pa-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   pa-all-districts-data.json (created/overwritten in the same directory)
...
const STATE_FIPS   = "42"; // Pennsylvania
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "pa-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "pa-all-districts-data.json");

  console.log("Loading election data from pa-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`pa-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  console.log("Fetching Census data for House districts 1–203 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~45 seconds\n");

  const houseNums   = Array.from({ length: 203 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Pennsylvania House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in pa-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Pennsylvania Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in pa-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/203  |  Election: ${houseElecOk}/203`);
