#!/usr/bin/env node
// hi-fetch-all.js
// Fetches ACS 5-year Census data for all 51 Hawaii House districts and all
// 25 Hawaii Senate districts, merges with pre-extracted election data from
// hi-elections-data.json, and saves to hi-all-districts-data.json.
//
// Usage:  node hi-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   hi-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "hi-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "hi-all-districts-data.json");

  console.log("Loading election data from hi-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`hi-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }

  // hi-elections-data.json stores districts as a flat array of records keyed
  // by districtId ("hi-hd-1".."hi-hd-51", "hi-sd-1".."hi-sd-25"); re-key by
  // the bare district number for lookup below.
  const electionList = JSON.parse(fs.readFileSync(ELECTION_FILE, "utf8"));
  const houseElection  = {};
  const senateElection = {};
  for (const d of electionList) {
    const houseMatch  = d.districtId.match(/^hi-hd-(\d+)$/);
    const senateMatch = d.districtId.match(/^hi-sd-(\d+)$/);
  ...
  console.log("Fetching Census data for House districts 1–51 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~15 seconds\n");

  const houseNums   = Array.from({ length: 51 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  console.log("\nFetching Census data for Senate districts 1–25 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~8 seconds\n");

  const senateNums   = Array.from({ length: 25 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Hawaii House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in hi-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Hawaii Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in hi-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/51  |  Election: ${houseElecOk}/51`);
  console.log(`  Senate — Census: ${senateCensusOk}/25  |  Election: ${senateElecOk}/25`);
  console.log(`  Total: ${houseCensusOk + senateCensusOk}/76`);
