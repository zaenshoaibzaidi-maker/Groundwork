#!/usr/bin/env node
// ok-fetch-all.js
// Fetches ACS 5-year Census data for all 101 Oklahoma House districts and all
// 48 Oklahoma Senate districts, merges with pre-extracted election data from
// ok-elections-data.json, and saves to ok-all-districts-data.json.
//
// Usage:  CENSUS_API_KEY=<key> node ok-fetch-all.js
// Requires: Node 18+ (native fetch)
// Output:   ok-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "ok-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "ok-all-districts-data.json");

  console.log("Loading election data from ok-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`ok-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  console.log("Fetching Census data for House districts 1–101 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~40 seconds\n");

  const houseNums   = Array.from({ length: 101 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  console.log("\nFetching Census data for Senate districts 1–48 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~15 seconds\n");

  const senateNums   = Array.from({ length: 48 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Oklahoma House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in ok-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Oklahoma Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in ok-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/101  |  Election: ${houseElecOk}/101`);
  console.log(`  Senate — Census: ${senateCensusOk}/48   |  Election: ${senateElecOk}/48`);
