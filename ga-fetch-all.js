#!/usr/bin/env node
// ga-fetch-all.js
// Fetches ACS 5-year Census data for all 180 Georgia House districts and all
// 56 Georgia Senate districts, merges with pre-extracted election data from
// ga-elections-data.json, and saves to ga-all-districts-data.json.
//
// Usage:  node ga-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   ga-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "ga-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "ga-all-districts-data.json");

  console.log("Loading election data from ga-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`ga-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  console.log("Fetching Census data for House districts 1–180 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~45 seconds\n");

  const houseNums   = Array.from({ length: 180 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  console.log("\nFetching Census data for Senate districts 1–56 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~15 seconds\n");

  const senateNums   = Array.from({ length: 56 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Georgia House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in ga-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Georgia Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in ga-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/180  |  Election: ${houseElecOk}/180`);
  console.log(`  Senate — Census: ${senateCensusOk}/56   |  Election: ${senateElecOk}/56`);
