#!/usr/bin/env node
// ok-generate-districts.js
// Reads ok-all-districts-data.json and generates ok-districts.js,
// structured identically to tx-districts.js.
//
// Usage: node ok-generate-districts.js
// Output: ok-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'ok-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'ok-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('ok-all-districts-data.json not found. Run ok-fetch-all.js first.');
  process.exit(1);
}
...
function genEntry(num, districtData, idPrefix, chamberLabel, type) {
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "Oklahoma ${chamberLabel} District ${num}",
    city: "TBD",
    region: "TBD",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "Oklahoma ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 101; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'ok-hd', 'House', 'state house district'));
}

const senateEntries = [];
for (let i = 1; i <= 48; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'ok-sd', 'Senate', 'state senate district'));
}

const output = `/* ok-districts.js — Groundwork Oklahoma District Data
 *
