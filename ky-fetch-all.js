#!/usr/bin/env node
// ky-fetch-all.js
// Fetches ACS 5-year Census data for all 100 Kentucky House districts and all
// 38 Kentucky Senate districts, merges with pre-extracted election data from
// ky-elections-data.json, and saves to ky-all-districts-data.json.
//
// Usage:  node ky-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   ky-all-districts-data.json (created/overwritten in the same directory)
...
// ky-elections-data.json stores house/senate as arrays of { districtId: "ky-hd-N" | "ky-sd-N", ... }.
function indexByDistrictNumber(districts, prefix) {
  const map = {};
  for (const d of districts || []) {
    const n = d.districtId.slice(prefix.length + 1); // strip "ky-hd-" / "ky-sd-"
    map[n] = d;
  }
  return map;
}
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "ky-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "ky-all-districts-data.json");

  console.log("Loading election data from ky-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`ky-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  const electionData   = JSON.parse(fs.readFileSync(ELECTION_FILE, "utf8"));
  const houseElection  = indexByDistrictNumber(electionData.house, "ky-hd");
  const senateElection = indexByDistrictNumber(electionData.senate, "ky-sd");
  console.log(
    `  House election districts: ${Object.keys(houseElection).length}`,
    `| Senate election districts: ${Object.keys(senateElection).length}\n`
  );

  console.log("Fetching Census data for House districts 1–100 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~20 seconds\n");

  const houseNums   = Array.from({ length: 100 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  console.log("\nFetching Census data for Senate districts 1–38 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~10 seconds\n");

  const senateNums   = Array.from({ length: 38 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Kentucky House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in ky-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Kentucky Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in ky-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/100  |  Election: ${houseElecOk}/100`);
  console.log(`  Senate — Census: ${senateCensusOk}/38   |  Election: ${senateElecOk}/38`);
