#!/usr/bin/env node
// ri-generate-districts.js
// Reads ri-all-districts-data.json and generates ri-districts.js,
// structured identically to ok-districts.js.
//
// Usage: node ri-generate-districts.js
// Output: ri-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'ri-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'ri-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('ri-all-districts-data.json not found. Run ri-fetch-all.js first.');
  process.exit(1);
}
...
function getDemRep(elec) {
  if (!elec || elec.error) return { dem: 0, rep: 0, ind: 0 };
  if (elec.unopposed) {
    if (elec.party === 'DEM') return { dem: 100, rep: 0, ind: 0 };
    if (elec.party === 'IND') return { dem: 0, rep: 0, ind: 100 };
    return { dem: 0, rep: 100, ind: 0 };
  }
  const pct = Math.round(elec.pct);
  if (elec.party === 'DEM') return { dem: pct, rep: 100 - pct, ind: 0 };
  if (elec.party === 'REP') return { dem: 100 - pct, rep: pct, ind: 0 };
  if (elec.party === 'IND') return { dem: 0, rep: 0, ind: pct };
...
function genEntry(num, districtData, idPrefix, chamberLabel, type) {
  ...
  const { dem, rep, ind } = getDemRep(elec);
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "Rhode Island ${chamberLabel} District ${num}",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "Rhode Island ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 75; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'ri-hd', 'House', 'state house district'));
}

const senateEntries = [];
for (let i = 1; i <= 38; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'ri-sd', 'Senate', 'state senate district'));
}

const output = `/* ri-districts.js — Groundwork Rhode Island District Data
 *
