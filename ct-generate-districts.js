#!/usr/bin/env node
// ct-generate-districts.js
// Reads ct-all-districts-data.json and generates ct-districts.js,
// structured identically to ok-districts.js.
//
// Usage: node ct-generate-districts.js
// Output: ct-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'ct-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'ct-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('ct-all-districts-data.json not found. Run ct-fetch-all.js first.');
  process.exit(1);
}
...
function genEntry(num, districtData, idPrefix, chamberLabel, type) {
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "Connecticut ${chamberLabel} District ${num}",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "Connecticut ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 151; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'ct-hd', 'House', 'state house district'));
}

const senateEntries = [];
for (let i = 1; i <= 36; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'ct-sd', 'Senate', 'state senate district'));
}

const output = `/* ct-districts.js — Groundwork Connecticut District Data
 *
 * Generated by ct-generate-districts.js from ct-all-districts-data.json.
 * Census: ACS 5-Year 2023 (2019–2023) | Elections: 2024 General (CT SOTS)
 * ─────────────────────────────────────────────────────────────────────── */

const DEMO_COLORS = ['#4e9e68','#2563eb','#f59e0b','#ef4444'];

const CT_HOUSE_DISTRICTS = [
${houseEntries.join(',\n')}
];

const CT_SENATE_DISTRICTS = [
${senateEntries.join(',\n')}
];
`;
