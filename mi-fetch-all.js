#!/usr/bin/env node
// mi-fetch-all.js
// Fetches ACS 5-year Census data for all 110 Michigan House districts and all
// 38 Michigan Senate districts, merges with pre-extracted election data from
// mi-elections-data.json, and saves to mi-all-districts-data.json.
//
// Usage:  node mi-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   mi-all-districts-data.json (created/overwritten in the same directory)
...
// mi-elections-data.json stores house/senate as arrays of { districtId: "mi-hd-N" | "mi-sd-N", ... }.
function indexByDistrictNumber(districts, prefix) {
  const map = {};
  for (const d of districts || []) {
    const n = d.districtId.slice(prefix.length + 1); // strip "mi-hd-" / "mi-sd-"
    map[n] = d;
  }
  return map;
}
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "mi-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "mi-all-districts-data.json");

  console.log("Loading election data from mi-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`mi-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  const electionData   = JSON.parse(fs.readFileSync(ELECTION_FILE, "utf8"));
  const houseElection  = indexByDistrictNumber(electionData.house, "mi-hd");
  const senateElection = indexByDistrictNumber(electionData.senate, "mi-sd");
  console.log(
    `  House election districts: ${Object.keys(houseElection).length}`,
    `| Senate election districts: ${Object.keys(senateElection).length}\n`
  );

  console.log("Fetching Census data for House districts 1–110 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~30 seconds\n");

  const houseNums   = Array.from({ length: 110 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Michigan House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in mi-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Michigan Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in mi-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/110  |  Election: ${houseElecOk}/110`);
