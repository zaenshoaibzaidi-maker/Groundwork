#!/usr/bin/env node
// mo-fetch-all.js
// Fetches ACS 5-year Census data for all 163 Missouri House districts and all
// 34 Missouri Senate districts, merges with pre-extracted election data from
// mo-elections-data.json, and saves to mo-all-districts-data.json.
//
// Usage:  node mo-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   mo-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "mo-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "mo-all-districts-data.json");

  console.log("Loading election data from mo-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`mo-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  console.log("Fetching Census data for House districts 1–163 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~33 seconds\n");

  const houseNums   = Array.from({ length: 163 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  console.log("\nFetching Census data for Senate districts 1–34 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~7 seconds\n");

  const senateNums   = Array.from({ length: 34 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Missouri House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in mo-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Missouri Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in mo-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/163  |  Election: ${houseElecOk}/163`);
  console.log(`  Senate — Census: ${senateCensusOk}/34   |  Election: ${senateElecOk}/34`);
