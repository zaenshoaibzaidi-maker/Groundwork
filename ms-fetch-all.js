#!/usr/bin/env node
// ms-fetch-all.js
// Fetches ACS 5-year Census data for all 122 Mississippi House districts and all
// 52 Mississippi Senate districts, merges with pre-extracted election data from
// ms-elections-data.json, and saves to ms-all-districts-data.json.
//
// Usage:  node ms-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   ms-all-districts-data.json (created/overwritten in the same directory)
...
const STATE_FIPS   = "28"; // Mississippi
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "ms-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "ms-all-districts-data.json");

  console.log("Loading election data from ms-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`ms-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  console.log("Fetching Census data for House districts 1–122 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~30 seconds\n");

  const houseNums   = Array.from({ length: 122 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  console.log("\nFetching Census data for Senate districts 1–52 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~15 seconds\n");

  const senateNums   = Array.from({ length: 52 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Mississippi House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in ms-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Mississippi Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in ms-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/122  |  Election: ${houseElecOk}/122`);
  console.log(`  Senate — Census: ${senateCensusOk}/52   |  Election: ${senateElecOk}/52`);
