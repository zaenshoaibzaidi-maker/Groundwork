#!/usr/bin/env node
// il-fetch-all.js
// Fetches ACS 5-year Census data for all 118 Illinois House districts and all
// 59 Illinois Senate districts, merges with pre-extracted election data from
// il-elections-data.json, and saves to il-all-districts-data.json.
//
// Usage:  node il-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   il-all-districts-data.json (created/overwritten in the same directory)
...
// il-elections-data.json stores house/senate as arrays of { districtId: "il-hd-N" | "il-sd-N", ... }.
function indexByDistrictNumber(districts, prefix) {
  const map = {};
  for (const d of districts || []) {
    const n = d.districtId.slice(prefix.length + 1); // strip "il-hd-" / "il-sd-"
    map[n] = d;
  }
  return map;
}
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "il-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "il-all-districts-data.json");

  console.log("Loading election data from il-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`il-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  const electionData   = JSON.parse(fs.readFileSync(ELECTION_FILE, "utf8"));
  const houseElection  = indexByDistrictNumber(electionData.house, "il-hd");
  const senateElection = indexByDistrictNumber(electionData.senate, "il-sd");
  console.log(
    `  House election districts: ${Object.keys(houseElection).length}`,
    `| Senate election districts: ${Object.keys(senateElection).length}\n`
  );

  console.log("Fetching Census data for House districts 1–118 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~25 seconds\n");

  const houseNums   = Array.from({ length: 118 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  console.log("\nFetching Census data for Senate districts 1–59 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~15 seconds\n");

  const senateNums   = Array.from({ length: 59 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Illinois House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in il-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Illinois Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in il-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/118  |  Election: ${houseElecOk}/118`);
  console.log(`  Senate — Census: ${senateCensusOk}/59   |  Election: ${senateElecOk}/59`);
