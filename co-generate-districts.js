#!/usr/bin/env node
// co-generate-districts.js
// Reads co-all-districts-data.json and generates co-districts.js,
// structured identically to tx-districts.js.
//
// Usage: node co-generate-districts.js
// Output: co-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'co-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'co-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('co-all-districts-data.json not found. Run co-fetch-all.js first.');
  process.exit(1);
}
...
function getDemRep(elec) {
  ...
  const unopposed = elec.other_candidates.length === 0;
  if (unopposed) {
    return elec.winner_party === 'D' ? { dem: 100, rep: 0 } : { dem: 0, rep: 100 };
  }
  const pct = Math.round(elec.winner_pct);
  if (elec.winner_party === 'D') return { dem: pct, rep: 100 - pct };
  if (elec.winner_party === 'R') return { dem: 100 - pct, rep: pct };
  return { dem: 0, rep: 0 };
}
...
function getPartisanSub(elec, incumbentName) {
  ...
  const unopposed = elec.other_candidates.length === 0;
  if (unopposed) return `${last} won unopposed in ${elec.year}.`;
  return `${last} won with ${Math.round(elec.winner_pct)}% in ${elec.year}.`;
}
...
function genEntry(num, districtData, idPrefix, chamberLabel, type) {
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "Colorado ${chamberLabel} District ${num}",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "Colorado ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 65; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'co-hd', 'House', 'state house district'));
}

const senateEntries = [];
for (let i = 1; i <= 35; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'co-sd', 'Senate', 'state senate district'));
}

const output = `/* co-districts.js — Groundwork Colorado District Data
 *
 * Generated by co-generate-districts.js from co-all-districts-data.json.
 * Census: ACS 5-Year 2023 (2019–2023) | Elections: 2024/2022 General (CO SoS)
 * ─────────────────────────────────────────────────────────────────────── */

const DEMO_COLORS = ['#4e9e68','#2563eb','#f59e0b','#ef4444'];

const CO_HOUSE_DISTRICTS = [
${houseEntries.join(',\n')}
];

const CO_SENATE_DISTRICTS = [
${senateEntries.join(',\n')}
];
`;
