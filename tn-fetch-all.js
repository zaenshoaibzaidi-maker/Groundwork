#!/usr/bin/env node
// tn-fetch-all.js
// Fetches ACS 5-year Census data for all 99 Tennessee House districts and all
// 33 Tennessee Senate districts, merges with pre-extracted election data from
// tn-elections-data.json, and saves to tn-all-districts-data.json.
//
// Usage:  node tn-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   tn-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "tn-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "tn-all-districts-data.json");

  console.log("Loading election data from tn-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`tn-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  console.log("Fetching Census data for House districts 1–99 (lower chamber)...");
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Tennessee House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in tn-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Tennessee Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in tn-elections-data.json" },
    };
  }
  ...
  const totalDistr = houseNums.length + senateNums.length;
  ...
  console.log(`  Total Census fetches: ${totalOk}/${totalDistr}`);
