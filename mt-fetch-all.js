#!/usr/bin/env node
// mt-fetch-all.js
// Fetches ACS 5-year Census data for all 100 Montana House districts and all
// 50 Montana Senate districts, merges with pre-extracted election data from
// mt-elections-data.json, and saves to mt-all-districts-data.json.
//
// Usage:  node mt-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   mt-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "mt-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "mt-all-districts-data.json");

  console.log("Loading election data from mt-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`mt-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  const electionRows   = JSON.parse(fs.readFileSync(ELECTION_FILE, "utf8"));
  const houseElection  = {};
  const senateElection = {};
  for (const row of electionRows) {
    const m = /^(HD|SD)-(\d+)$/.exec(row.district || "");
    if (!m) continue;
    const num = String(parseInt(m[2], 10));
    if (m[1] === "HD") houseElection[num] = row;
    else senateElection[num] = row;
  }
  console.log(
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Montana House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in mt-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Montana Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in mt-elections-data.json" },
    };
  }
