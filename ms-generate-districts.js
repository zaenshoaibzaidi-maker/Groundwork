#!/usr/bin/env node
// ms-generate-districts.js
// Reads ms-all-districts-data.json and generates ms-districts.js,
// structured identically to ok-districts.js / tx-districts.js.
//
// Usage: node ms-generate-districts.js
// Output: ms-districts.js (created/overwritten in the same directory)
...
const INPUT  = path.join(__dirname, 'ms-all-districts-data.json');
const OUTPUT = path.join(__dirname, 'ms-districts.js');

if (!fs.existsSync(INPUT)) {
  console.error('ms-all-districts-data.json not found. Run ms-fetch-all.js first.');
  process.exit(1);
}
...
function getPartisanSub(elec, incumbentName) {
  ...
  if (elec.unopposed) return `${last} won unopposed in 2023.`;
  return `${last} won with ${Math.round(elec.pct)}% in 2023.`;
}
...
function genEntry(num, districtData, idPrefix, chamberLabel, type) {
  ...
  return `  {
    id: "${idPrefix}-${num}",
    name: "Mississippi ${chamberLabel} District ${num}",
    city: "TBD",
    region: "TBD",
    type: "${type}",
    ...
    seatStatus: "Active",
    dashboard: {
      subtitle: "Mississippi ${chamberLabel} District ${num}",
      chips: [],
      stats: [
...
const houseEntries = [];
for (let i = 1; i <= 122; i++) {
  houseEntries.push(genEntry(i, data.house[String(i)], 'ms-hd', 'House', 'state house district'));
}

const senateEntries = [];
for (let i = 1; i <= 52; i++) {
  senateEntries.push(genEntry(i, data.senate[String(i)], 'ms-sd', 'Senate', 'state senate district'));
}

const output = `/* ms-districts.js — Groundwork Mississippi District Data
 *
 * Generated by ms-generate-districts.js from ms-all-districts-data.json.
 * Census: ACS 5-Year 2023 (2019–2023) | Elections: 2023 General (MS SoS)
 * ─────────────────────────────────────────────────────────────────────── */

const DEMO_COLORS = ['#4e9e68','#2563eb','#f59e0b','#ef4444'];

const MS_HOUSE_DISTRICTS = [
${houseEntries.join(',\n')}
];

const MS_SENATE_DISTRICTS = [
${senateEntries.join(',\n')}
];
`;
