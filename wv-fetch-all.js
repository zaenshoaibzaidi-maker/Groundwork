#!/usr/bin/env node
// wv-fetch-all.js
// Fetches ACS 5-year Census data for all 100 West Virginia House districts and all
// 17 West Virginia Senate districts, merges with pre-extracted election data from
// wv-elections-data.json, and saves to wv-all-districts-data.json.
//
// Usage:  node wv-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   wv-all-districts-data.json (created/overwritten in the same directory)
...
const STATE_FIPS   = "54"; // West Virginia
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "wv-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "wv-all-districts-data.json");

  console.log("Loading election data from wv-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`wv-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  console.log("\nFetching Census data for Senate districts 1–17 (upper chamber)...");
  console.log("  200ms delay between each request — est. ~5 seconds\n");

  const senateNums   = Array.from({ length: 17 }, (_, i) => i + 1);
  const senateCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `West Virginia House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in wv-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `West Virginia Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in wv-elections-data.json" },
    };
  }
  ...
  console.log(`  Senate — Census: ${senateCensusOk}/17   |  Election: ${senateElecOk}/17`);
