#!/usr/bin/env node
// de-fetch-all.js
// Fetches ACS 5-year Census data for all 41 Delaware House districts and all
// 21 Delaware Senate districts, merges with pre-extracted election data from
// de-elections-data.json, and saves to de-all-districts-data.json.
// (de-all-districts-data.json feeds de-generate-districts.js → de-districts.js)
//
// Usage:  node de-fetch-all.js   (reads CENSUS_API_KEY from .env)
// Requires: Node 18+ (native fetch)
// Output:   de-all-districts-data.json (created/overwritten in the same directory)
...
async function main() {
  const ELECTION_FILE = path.join(__dirname, "de-elections-data.json");
  const OUTPUT_FILE   = path.join(__dirname, "de-all-districts-data.json");

  console.log("Loading election data from de-elections-data.json...");
  if (!fs.existsSync(ELECTION_FILE)) {
    console.error(`de-elections-data.json not found in ${__dirname}.`);
    process.exit(1);
  }
  ...
  console.log("Fetching Census data for House districts 1–41 (lower chamber)...");
  console.log("  200ms delay between each request — est. ~10 seconds\n");

  const houseNums   = Array.from({ length: 41 }, (_, i) => i + 1);
  const houseCensus = {};
  ...
  for (const n of houseNums) {
    house[String(n)] = {
      districtNumber: n,
      districtName:   `Delaware House District ${n}`,
      census: houseCensus[n].ok ? houseCensus[n].data : { error: houseCensus[n].error },
      election: houseElection[String(n)] || { error: "District not found in de-elections-data.json" },
    };
  }
  ...
  for (const n of senateNums) {
    senate[String(n)] = {
      districtNumber: n,
      districtName:   `Delaware Senate District ${n}`,
      census: senateCensus[n].ok ? senateCensus[n].data : { error: senateCensus[n].error },
      election: senateElection[String(n)] || { error: "District not found in de-elections-data.json" },
    };
  }
  ...
  console.log("\nComplete.");
  console.log(`  House  — Census: ${houseCensusOk}/41  |  Election: ${houseElecOk}/41`);
